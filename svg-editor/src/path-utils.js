...
export function serializeD(commands) {
  ...
  return commands.map(cmd => {
    ...
    return cmd.letter + keys.map(k => fmtNum(cmd[k])).join(' ');
  }).join(' ');
}

export function computePositions(commands) {
  let cx = 0, cy = 0, initX = 0, initY = 0, prevCpX = null, prevCpY = null;
  return commands.map(cmd => {
    const { letter } = cmd;
    ...
    if (L === 'M') {
      ...
      initX = absX; initY = absY; prevCpX = null; prevCpY = null;
    } else if (L === 'L' || L === 'T') {
      absX = r(cx, cmd.x); absY = r(cy, cmd.y); prevCpX = null; prevCpY = null;
    } else if (L === 'H') {
      absX = r(cx, cmd.x); absY = cy; prevCpX = null; prevCpY = null;
    } else if (L === 'V') {
      absX = cx; absY = r(cy, cmd.y); prevCpX = null; prevCpY = null;
    } else if (L === 'C') {
      ...
      absX = r(cx, cmd.x); absY = r(cy, cmd.y);
      controls = [{ x: cpx, y: cpy }]; prevCpX = cpx; prevCpY = cpy;
    } else if (L === 'A') {
      absX = r(cx, cmd.x); absY = r(cy, cmd.y); prevCpX = null; prevCpY = null;
    } else if (L === 'Z') {
      absX = initX; absY = initY; prevCpX = null; prevCpY = null;
    }
    cx = absX; cy = absY;
...
function isLetter(s) { return /^[MmZzLlHhVvCcSsQqTtAa]$/.test(s); }
