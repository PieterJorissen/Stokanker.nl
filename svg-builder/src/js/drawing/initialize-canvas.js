...
import { renderUnderlayElement } from '../underlay-handling';
...
function initializeCanvas() {
    ...
    // prepend underlay image below all layer elements
    renderUnderlayElement(drawingContent);
    // rm and create cps
    ...
    // sync underlay UI controls to current drawing state
    const opacitySlider = document.getElementById('underlay-opacity');
    const exportToggle = document.getElementById('underlay-export-toggle');
    if (opacitySlider) opacitySlider.value = drawing.underlay.opacity;
    if (exportToggle) exportToggle.checked = drawing.underlay.includeInExport;
}