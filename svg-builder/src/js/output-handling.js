...
const underlayImageRe = /<image id="underlay-image"[^>]*><\/image>|<image id="underlay-image"[^/]*\/>/g;
...
function generateMarkUp() {
    let content = drawingContent.innerHTML;
    if (!drawing.underlay.includeInExport) {
        content = content.replace(underlayImageRe, '');
    }
    ...
    <g transform="${stringifyTransforms(drawing.transforms)}">${content}</g></svg>`
        .replace(layerIdRe, '')
